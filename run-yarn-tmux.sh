...
$TMUX -f "$YAY_PATH"/tmux.conf new-session -d -s dev -n "build"

# Create a new window for tracking file commands
$TMUX new-window -n "trackers"

for DEP_PATH in "${DEP_PATHS[@]}"; do
	# Track src/TS(X), JS and JSON files
	FILES_TO_TRACK="find $DEP_PATH/src -type f \( -name '*.ts' -o -name '*.tsx' -o -name '*.js' -o -name '*.json' \)"

	# Create a new pane for each DEP_PATH in the "trackers" window, except for the first one
	# The first one is added to newly created window 'trackers' out of the loop
	...
	dependents_json=$(printf '%s\n' "${dependents[@]}" | jq -R . | jq -s .)

	# Remove unneccessary transitive dependents
	transitive_dependencies=$(jq --argjson trackedDirs "$tracked_dirs_json" --arg allDependents "$dependents" \
		'.[] | select(.location as $loc | $allDependents | index($loc)) | .dependents[] | select(. as $dep | $trackedDirs | index($dep))' \
		"$APP_ROOT/dep-graph.json")
	filtered_dependents=$(
		echo "$dependents_json" | jq --arg transitives "$transitive_dependencies" \
			'.[] | select(. as $dep | $transitives | index($dep) | not)'
	)

	DEPENDENT_LOGS=()

	# Iterate over each dependent and create the DEPENDENT_LOG path
	for path in $filtered_dependents; do
	...
	UPDATE_DEPENDENT_LOGS="echo $DEP_DIR | tee -a $DEPENDENT_LOGS > /dev/null"

	# Command to monitor yarn output and update the PROJECT_LOG file on compilation success
	NOTIFY_ON_BUILD="$YAY_PATH/notify-on-complete.sh \"yarn start\" \"Compiled successfully\" \"$UPDATE_DEPENDENT_LOGS\""
	...
	$TMUX send-keys -t dev:trackers."$LAST_PANE_INDEX" "$FILES_TO_TRACK | entr -z -p $RUN_YARN_IN_NEW_PANE && echo $DEP_LOG | entr -r timeout -s 2 0 $NOTIFY_ON_BUILD" C-m
...
$TMUX send-keys -t dev:build.0 "cd $PROJECT_PATH && echo $PROJECT_LOG | entr -r timeout -s 2 0 yarn start" C-m
